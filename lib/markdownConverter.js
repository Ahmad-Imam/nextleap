...
export function skillsToMarkdown(skills = []) {
  ...
  skills.forEach((skill) => {
    ...
    const rawType = String(skill?.type || "General").trim();
    const type = rawType || "General";

    if (!groupedSkills.has(type)) {
      groupedSkills.set(type, []);
    }
    groupedSkills.get(type).push(name);
  });

  if (!groupedSkills.size) return "";
  ...
  return `<section style="margin:24px 0;">
    ...
    <ul style="margin:0; padding-left:18px; color:#111827;">
      ${Array.from(groupedSkills.entries())
        .map(([type, skillNames]) => {
          const formattedNames = skillNames.map((name) => escapeHtml(name)).join(", ");
          return `<li style="margin-bottom:6px;"><strong>${escapeHtml(type)}:</strong> ${formattedNames}</li>`;
        })
        .join("")}
    </ul>
  </section>`;
